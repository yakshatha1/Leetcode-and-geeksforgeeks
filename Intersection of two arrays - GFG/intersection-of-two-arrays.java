...
class Solution {
    ...
    public static int NumberofElementsInIntersection(int a[], int b[], int n, int m) {
        ...
        // Add all elements from array a[] into the HashSet
        for (int i = 0; i < n; i++) {
            set.add(a[i]);
        }
        ...
        // Check each element in array b[] if it exists in the HashSet
        for (int i = 0; i < m; i++) {
            if (set.contains(b[i])) {
                count++;
                set.remove(b[i]); // Remove the element from the set to avoid counting duplicates
            }
        }
